...
public class DateUtils {
	...
	// add "\u5e74 \u6708 \u65e5" for Chinese/Japanese date format:2017 \u5e74 2
	// \u6708 7 \u65e5
	private static final Pattern date_ptrn3b = Pattern.compile("^[\\[\\]yYmMdDhHsS\\-T/\u5e74\u6708\u65e5,. :\"\\\\]+0*[ampAMP/]*$");
	// elapsed time patterns: [h],[m] and [s]
	private static final Pattern date_ptrn4 = Pattern.compile("^\\[([hH]+|[mM]+|[sS]+)\\]");
	// for format which start with "[DBNum1]" or "[DBNum2]" or "[DBNum3]" could be a
	// Chinese date
	private static final Pattern date_ptrn5 = Pattern.compile("^\\[DBNum(1|2|3)\\]");
	// for format which start with "年" or "月" or "日" or "时" or "分" or "秒" could be a
	// Chinese date
	...
	public static String format(BigDecimal date, Boolean use1904windowing, String dateFormat) {
		...
		LocalDateTime localDateTime = DateUtil.getLocalDateTime(date.doubleValue(), BooleanUtils.isTrue(use1904windowing), true);
	...
	/**
	 * Given an Excel date with either 1900 or 1904 date windowing, converts it to a
	 * java.util.Date.
	 *
	 * Excel Dates and Times are stored without any timezone information. If you
	 * know (through other means) that your file uses a different TimeZone to the
	 * system default, you can use this version of the getJavaDate() method to
	 * handle it.
	 *
	 * @param date             The Excel date.
	 * @param use1904windowing true if date uses 1904 windowing, or false if using
	 *                         1900 date windowing.
	 * @return Java representation of the date, or null if date is not a valid Excel
	 *         date
	 */
	public static Date getJavaDate(double date, boolean use1904windowing) {
		// To calculate the Date, in the use of
		// `org.apache.poi.ss.usermodel.DateUtil.getJavaDate(double, boolean,
		// java.util.TimeZone, boolean), Date when similar `2023-01-01 00:00:00.500`,
		// returns the`2023-01-01
		// 00:00:01`, but excel in fact shows the `2023-01-01 00:00:00`.
		// `org.apache.poi.ss.usermodel.DateUtil.getLocalDateTime(double, boolean,
		// boolean)` There is no problem.
