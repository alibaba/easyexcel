 ...
 * @author jipengfei
 */
public class ExcelHeadProperty implements Serializable {
	private static final long serialVersionUID = 842533386186092181L;
	...
	private int getLastRangNum(int j, String value, List<String> values) {
		...
		for (int i = last + 1; i < values.size(); i++) {
			...
			if (value.equals(current)) {
				...
				// if i>j && !value.equals(current) Indicates that the continuous range is
				// exceeded
				if (i > j) {
					break;
				}
